...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            mask_features = PyramidROIAlign([config.MASK_POOL_SIZE, config.MASK_POOL_SIZE],
                                            name="roi_align_mask_for_featureExt")(
                [detection_boxes, input_image_meta] + mrcnn_feature_maps)

            model = KM.Model([input_image, input_image_meta, input_anchors],
                             [detections, mrcnn_class, mrcnn_bbox,
                              mrcnn_mask, rpn_rois, rpn_class, rpn_bbox, mask_features],
                             name='mask_rcnn')
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        if exclude_ix.shape[0] > 0:
            ...
            feats = np.delete(feats, exclude_ix, axis=0)
            N = class_ids.shape[0]
        ...
        return boxes, class_ids, scores, full_masks, feats
    ...
    def detect(self, images, verbose=0):
        ...
        detections, _, _, mrcnn_mask, _, _, _, mrcnn_feat = \
            self.keras_model.predict([molded_images, image_metas, anchors], verbose=0)
        # Process detections
        results = []
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks, final_feats = \
                self.unmold_detections(detections[i], mrcnn_mask[i],
                                       image.shape, molded_images[i].shape,
                                       windows[i], mrcnn_feat[i])
            results.append({
                "rois": final_rois,
